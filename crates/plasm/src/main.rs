...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Start { config, listen, peer } => {
            ...
            // Load (or, on first run, generate-and-persist) the node's
            // Ed25519 identity. This fixes the long-standing ephemeral-key
            // bug — peer-id is now stable across restarts.
            ...
            // Start listening on configured addresses. After phase-core M2
            // these are async — `Discovery` drives the swarm on an internal
            // task and the public API is command-channel based.
            for addr in &listen_addrs {
                discovery.listen(addr).await?;
            }
            ...
            for peer_addr in &cfg.peer_addrs {
                ...
                if let Err(e) = discovery.dial_peer(peer_addr).await {
